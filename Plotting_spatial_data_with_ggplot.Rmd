...
subtitle: "Plotting spatial data with ggplot"
...
While you can create plots through various ways, including base `R`, the most popular method of producing fancy figures is with the `ggplot2` package. You can plot **raster** and **vector** spatial data with `ggplot.` Vector data represent geographical phenomena by *points*, *lines* and *polygons*. Raster is another representation of spatial data that consist of pixels.   
...
ggplot(data = au) +
  geom_sf(size = .3, fill = "white") + # change the line size and polygon colours
  theme_minimal() +
  coord_sf(crs = 3112) + 
...
sam <- sampleRegular(au_temp, 5e5, asRaster = TRUE) %>% 
...
You can change the colour and other parametrs of the plot to make it better.   
It is important to use `coord_sf` or `coord_fixed` to keep the dimention of the plot fixed.

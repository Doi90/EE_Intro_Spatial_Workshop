...
Here are 3 examples of possible combinations, but of course there are many more (For example ESRI (the company behind ARC-GIS) have an extensive online help that explains many concepts and tools with text and pictures).
...
To find out, we have to "intersect" the 2 different layers with each other, to find out how they relate with each other. This can be done with the command "extract()" from the raster package. In case you have not done so, read in the shapefile of the Australian states. 
...
don't have to worry about summarizing data. If you have not done so yet, read in the koala data again.
You also want to check if they are in the same coordinate reference system before you do you analysis.
...
When you extract data to polygons, like the states of Australia, you have potentially a lot of different values within each state, so we have to decide what we want to do with these values
...
# Challenge 3: Count points in polygons
class.counts$OBJECTID <- as.numeric(rownames(class.counts)) # 1:8 not recommended
...
(This is more or less what you already did in the vector part and just here for repetition)

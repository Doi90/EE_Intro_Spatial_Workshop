...
feature1 <- rbind(c(1, 1.0), 
                  c(1.5, 2), 
                  c(2, 2), 
                  c(2.5, 1.5))

feature2 <- rbind(c(1, 1.0), 
                  c(1.5, 2), 
                  c(2, 2), 
                  c(2.5, 1.5), 
                  c(1, 1.0))
...
![](images/simple_features.png)

The structure of the data in `R` is as follow:

![](images/sf_xfig.png)

We highly recommed reading [the simple feature web page](https://r-spatial.github.io/sf/articles/sf1.html).   
...
big_cities <- filter(cities, 
                     pop > 1e6)

plot(australia$geometry)

plot(big_cities["pop"], 
     cex = big_cities$pop / 1e6, 
     lwd = 2, 
     add = TRUE)
```
...
```{r message=FALSE, warning=FALSE}
australia_clip <- st_crop(australia, 
                          ext)
...
```{r}
koala_states <- st_intersection(koala_proj, 
                                states)
...
```{r}
koala_dist <- st_distance(koala_proj, 
                          koala_proj)
